...
public class XmppIoHandler implements IoHandler {
	...
	public void sessionCreated(IoSession session) throws Exception {
		log.debug("sessionCreated()...,session is:" + session);
		// session.getConfig().setBothIdleTime(120);
	...
	public void sessionOpened(IoSession session) throws Exception {
		log.debug("sessionOpened()...,session is:" + session);
	...
	//无论是后动关闭session还是，系统抛异常导致的session关闭，在session关闭后都要调用此方法，
	//我们可以在此方法中将ioSession中保存的未发送成功的message持久化到数据库中。
	public void sessionClosed(IoSession session) throws Exception {
		saveUnDeliver(session);
		log.debug("sessionClosed()...,session is:" + session);
...
//	private void saveUnDeliver(IoSession session) {
//		List<Message> msgList = (List<Message>) session.getAttribute(NotificationManager.MSG_KEY);
//		if (msgList != null) {
...
//	}
	private void saveUnDeliver(IoSession session) {
		Map<String,Message> msgs=(Map<String, Message>) session.getAttribute(NotificationManager.MSG_KEY);
		if (msgs != null) {
			messageService.saveBatch(msgs.values());
			msgs.clear();
		}
	}
	...
	public void exceptionCaught(IoSession session, Throwable cause)throws Exception {
		...
		log.debug("exceptionCaught()...,session is:" + session);
...
//		}
	...
	public void messageReceived(IoSession session, Object message)
			throws Exception {
		log.debug("messageReceived(),RCVD: " + message+",session is:"+session);
	...
	public void messageSent(IoSession session, Object message) throws Exception {
		log.debug("messageSent()...,session is:" + session);
