...
public class PresenceUpdateHandler {
    ...
    public PresenceUpdateHandler() {
        ...
        notificationManager=(NotificationManager) ServiceLocator.getService("notificationManager");
    ...
    public void process(Packet packet) {
    	log.debug("---------------------------PresenceUpdateHandler"+packet.toXML());
        ClientSession session = sessionManager.getSession(packet.getFrom());

        try {
            Presence presence = (Presence) packet;
            Presence.Type type = presence.getType();
            log.debug("---------------------------PresenceUpdateHandler,Persence.Type is:"+type);
            ...
            if (type == null) { // null == available 
                ...
                if (session != null) {
                    ...
                    // 验证成功后，将滞留的Message发送给用户,更新用户的上次登录时间为当前时间。
                    notificationManager.sendRetentionNotificationToUser(Config.getString("apiKey","1234567890"), session.getUsername());
        ...
        } catch (Exception e) {log.error("Internal server error. Triggered by packet: " + packet,e);
        }
    }
